# Recon Map 产出规范

> 被 [project-recon](SKILL.md) 引用的披露参考。Recon Map 核心是写给项目新读者看的——让一个人能在最短时间内抓住关键信息。只在你需要写文件或检查格式时查阅本节。

## 读者导航

Recon Map 按阅读优先级组织。建议阅读顺序：

1. **RECON.md — Overview + Module Coverage 表**：项目全貌、解决什么问题、技术栈、模块划分
2. **user-journeys.md**：用户怎么用这个系统，每步由哪些负责实体承载（→ modules/{m}.md、architecture.md、glossary.md 查看详情）
3. **glossary.md**：项目专用术语——看不懂领域名词时查阅
4. **选一个最关心的模块，读其 局部旅程**：主功能走通，入口到出口
5. **architecture.md**：模块间关系和核心抽象（聚焦"改哪里影响谁"时读）
6. **dev-loop.md**：搭建环境时查阅

每份文件独立可读——不需要从头到尾读完。

产出文件间按需建立交叉引用，帮助读者跳到相关信息。不必每份都加——只在跳转能节省读者查找时间时加。

## 文件结构

目标项目根目录下 **`.recon/`** 子目录，集中存放所有产出物：

```
{目标项目}/
└── .recon/
    ├── RECON.md                 ← Overview + 覆盖状态表（唯一索引）
    ├── glossary.md              ← 项目专用术语
    ├── architecture.md          ← 模块依赖图（含外部系统）+ 核心抽象 + 数据模型
    ├── user-journeys.md         ← 用户旅程（每步标注负责实体 → 模块/服务/层/外部 API）
    ├── entry-points.md          ← 运行时入口清单（main、CLI、路由注册、导出API）+ 运行时配置
    ├── dev-loop.md              ← 构建/测试/lint/CI 命令与环境搭建
    ├── design-decisions.md      ← 设计决策（跨模块统一归档，深入了解模块时产出）
    └── modules/                 ← 模块专属文件（Step 3 创建骨架，Step 4/6 增补）
        ├── core.md              ← 职责/上下游 + 局部旅程 + Deep 节
        ├── auth.md
        ├── payment.md
        └── {subsystem}/         ← 大型项目：按子系统分组
            └── {module}.md
```

不写 `.gitignore`——Recon Map 默认进入版本控制，可被团队共享和审查。如果决定后续排除，一行 `.recon/` 到 `.gitignore` 即可。

## user-journeys.md 格式

```markdown
# User Journeys: {project-name}

## 用户画像
| 画像 | 角色 | 目标 |
|------|------|------|
| {persona} | {一句话角色} | {要达成什么} |

## 旅程：{name}
> {一句话——谁、什么场景、达成什么}

1. {进展节点} → {负责实体：模块/服务/层/外部 API/子系统}
2. {进展节点}

完成标志：{什么情况下算做完了}
```

步骤标注使用 `→ {负责实体}` 标注在步骤之后。每一步命名的是用户推进价值的里程碑，不是界面操作。一个文件可有多个旅程，每个旅程是一个连贯的端到端路径。

（首次侦察时产出。后续用户指向具体模块时按需补充。）

**写不出"用户做了一步得到某个结果，然后做下一步得到另一个结果"的线——说明这不是一个旅程，是功能清单。要么拆成多个独立旅程，要么归为其他旅程的上游或下游环节。**

每个模块一个 `.recon/modules/{module}.md`。

```markdown
# {module}

{职责：这个模块做什么、解决什么问题。
 上下游：谁依赖它、它依赖谁——模块间依赖方向用 → 表示。}

## 局部旅程（按需）

{内容在模块中如何流动——数据形态变化、代码执行路径，或用户与模块的交互。
 沿途提及理解流程必需的关键数据结构，一两行足矣，不必展开。
 代码跟踪需精确到 文件名::函数名。无则标记 ○。}

## Deep

### 内部结构（设计拆解）

{模块由哪些子组件构成，它们之间的依赖/连接关系。
 不是代码轨迹（不跟踪执行路径），也不是类图（不过度展开实现细节）。
 是模块内部的"架构图"——帮助读者在脑中建立模块的内部地图。}

### 设计决策

{为什么这样设计——核心抽象的选择理由、关键 tradeoff、被放弃的替代方案、代码证据。
 不是"怎么实现的"，是"为什么这样实现"。}
```

## 模块拆分

文件内容与文件标题的抽象层次一致。当局部旅程或 Deep 偏离了标题的抽象层（描述"模块内的某个东西"而非"模块本身"），迁到子文件 `modules/{模块}/{子实体}.md`。

拆分不靠行数或实体数量，靠内容与标题的抽象层是否对齐。

## RECON.md 格式

```markdown
# Recon Map: {project-name}
Generated: {date} | HEAD: {commit}

## Overview

- **定位**：{一句话——项目是什么 + 技术栈}
- **问题域**：{这个项目解决用户的什么问题——谁在什么场景下用它来干什么}
- **模块地图**：{module} → 一句话职责（→ modules/{m}.md）；…
- **关键架构决策**：{最值得注意的设计选择，不是全部}
- **快速开始**：{想跑起来做什么 → dev-loop.md}
- **推荐入口**：如果想理解架构，读 {产出/章节}；如果想动手改代码，读 {产出/章节}

## Module Coverage

| Module | 1.Perim | 2.Entry | 3.Arch | 4.Journey | 5.DevLoop | 6.Deep | Last Visit |
|--------|---------|---------|--------|--------|-----------|--------|------------|
| core/  | ✓       | ✓       | ✓      | ✓      | -         | ✓      | 2026-06-27 |
| auth/  | ✓       | ✓       | ✓      | ✓      | -         | -      | 2026-06-27 |
| payment/ | ✓     | ✓       | ✓      | -      | -         | -      | 2026-06-26 |

### 字段规则

**Module**：以代码目录/模块为粒度。一个模块一次覆盖整块代码。

**里程碑（1-6）**：`✓` = 完成该步骤且有内容；`○` = 已评估，无需写（如该模块没有数据旅程或代码轨迹）；`-` = 未完成。Agent 用 `-` 来判断尚未覆盖的工作。

> **4.Journey 列**：`✓` = 已写 局部旅程；`○` = 评估后该模块无需局部旅程（无数据旅程或代码轨迹）；`-` = 未评估。读者据符号即可判断，无需追到 `modules/{module}.md` 确认。

**Last Visit**：最后一次 revisit 的日期。超过 30 天且有最新 commit 变化则建议 revisit。

**子表**：大模块可展开为子表——主表该行标记 `→`，子表在同一文件下面另起一节。

```
| core/ | ✓ | ✓ | ✓ | → | - | ✓ | ← 标记 → 表示展开 |
|   skeleton | ✓ | ✓ | ✓ | ✓ | - | ✓ | ← 缩进表示子模块 |
|   adaptation | ✓ | ✓ | ✓ | ✓ | - | ✓ |
```
