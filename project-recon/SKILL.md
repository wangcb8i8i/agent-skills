...
> **校准**：与首次侦察（Step 1→2→3→4→5）不同，已有 RECON.md 基底。不从头走全流程，而是按用户指定粒度先读实际代码 → 再对比 RECON.md 差异 → 更新过时内容。三个粒度：函数/文件级读单个实现、模块级读核心文件后补局部旅程或 Deep、项目级快速回访关键流程。
...
- 校准后：按粒度先读代码 → 验证现 RECON.md 是否准确 → 更新局部旅程 + Deep
...
在用户要求深入了解指定模块时执行。Deep 的抽象层次遵守产出原则：文件标题是什么，Deep 就分析什么——先测绘内部结构，再评价设计决策。

| 动作 | 具体内容 |
|------|---------|
| 读 | 该模块的所有核心文件（不是全部，是核心） |
| 拆 | 模块内部组件——子模块、层、抽象边界，及其连接关系 |
...
**校准时**：必须先重新读核心代码文件，再将 Deep 节与代码比照更新，不得直接编辑 Deep 节而不验证代码。
