...
**完成标准**：能说出用户的典型旅程——"用户先做 X 才能做 Y，做完 Z 就结束了"——且每个步骤能对应到一个入口点（Step 2）或一个 Feature Flow（Step 4a）。
...
**完成标准**：能解释模块的内部设计、关键 tradeoffs、和"为什么这么设计"（每个 tradeoff 需附代码证据 `文件名::行号`）。同时更新术语表 `.recon/glossary.md`。
