...
3. 如果有未完成模块 → 对每个 X 走对应步骤（通常是 Step 4 Flow），创建或更新 `.recon/{module}.md` 的 Flow 节，完成后更新里程碑
...
3. 执行 Step 6（Deep Recon）于该模块：
   - 创建或更新 `.recon/{module}.md` 的 Deep 节
...
3. 更新 `.recon/{module}.md`（Flow 节 + Deep 节）
...
**最重要的步骤。** 选一个主要功能，**从入口到出口完整追踪**，产出到 `.recon/{module}.md` 的 Flow 节。
...
| 问 | 为什么这么设计 |
| 写 | Deep 节到 `.recon/{module}.md` + 设计决策追加到 `.recon/design-decisions.md` |
