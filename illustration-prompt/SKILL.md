...
读全文，提取领域、受众、文章类型、语气。标记需要图解的关键节点：抽象概念首次出现、架构说明、对比转折、数据描述、流程步骤、类比隐喻。为每个候选节点记录 source span，并标记以下维度：**概念密度**（高/中/低）、**理解增益**（高/中/低）、**叙事重要性**（高/中/低）、**视觉可译性**（高/中/低）、**图中文字敏感度**（无文字 / 仅标签 / 仅数字 / 必须精确文字）。候选节点按综合优先级排序，而不是只按概念密度排序。

### 3. OUTPUT FORM + MODEL ANCHORING — 产出形态与模型锚定

在 COMPREHEND 输出经用户确认后，进入风格推断前，先锚定产出形态、目标模型和全局图中文字策略。
...
- 全局图中文字策略: [无文字 / 仅标签 / 仅数字 / 必须精确文字 / 尽量无字，文案转移到图外]
- 特殊约束: [如有]
...
| 社交卡片 | 1 张，16:9 固定，强调标题 + 视觉冲击 | 装饰配图策略，横纵比 16:9，按图中文字策略处理标题 |
| 演示配图 | 按章节或关键节点分配，每张自包含 | 文字极少，视觉优先，动态感描述 |

### 4. STYLE ANCHOR — 推断风格

从语感/领域/受众推断统一视觉风格，并结合目标模型、优先目标和全局图中文字策略调整风格表达，输出结构化风格锚点摘要（色板、渲染风格、氛围、参考方向）。**提交用户确认**，用户可调整色板/渲染风格/氛围/参考方向或整体覆写。

### 5. PLANNING — 规划配图

基于综合优先级驱动，输出配图规划（位置、数量、每张的配图类型），每张图自动生成 diagram-slug，并为每张图建立 Source Grounding Ledger。**提交用户确认**，用户可增删位置、调整类型、修改 slug、重新排序，也可覆盖单图的图中文字策略。
...
- **关键节点**：以下位置优先标记为配图候选
  ...
  - 流程/步骤说明
  - 类比或隐喻（可用插图强化）
- **source span**：每个候选节点对应的原文位置（段落、节、小节或引用片段）
- **图中文字敏感度**：无文字 / 仅标签 / 仅数字 / 必须精确文字
...
将领域、受众、文章类型、语气及候选节点列表输出摘要提交用户确认。每个候选节点应至少包含：source span、综合优先级判断、概念密度、图中文字敏感度。用户可修正领域判断、增删候选节点、调整评分和文字敏感度。确认后方可进入 Step 3（产出形态与模型锚定）。
...
- Global on-image text policy: [No text / Labels only / Numbers only / Exact text required / Prefer no text]
```
...
- 图中文字策略默认全局生效；单图如需例外，在 PLANNING 阶段显式覆写。
...
将结构化摘要呈现给用户，用户确认时，可逐项修改或整体覆写。风格锚点必须与 Target Model Anchor 和全局图中文字策略一致。配色方案默认可参考 `references/tag-quick-reference.md`；按配图类型的写法细节见 `references/type-strategies.md`。
...
每张图规划时都要附带最小 Source Grounding Ledger：

```markdown
- Source span: [原文位置]
- Direct facts: [只写原文直接陈述的事实]
- Allowed inference: [允许做的轻量推断]
- Visual metaphor: [抽象概念对应的视觉隐喻]
- Must not invent: [不得擅自加入的元素]
- On-image text policy: [继承全局 / 单图覆写]
```

如果某张图需要把抽象概念翻译为视觉主意象，优先参考 `references/metaphor-guide.md` 选择隐喻，再写入 `Visual metaphor` 字段。
...
### On-image Text Policy → Prompt Strategy

| Text Policy | Prompt 策略 |
|-------------|------------|
...
- 目标模型: [Generic / GPT Image / Midjourney / Flux / SDXL / Ideogram / 其他]
- 图中文字策略: [无文字 / 仅标签 / 仅数字 / 必须精确文字 / 尽量无字]
...
1. **构建内部追踪数据**（不输出，用于校验）
   ...
   ```
    ...
    关系图: 实体间连接方式、方向、线型
    色板:   该图涉及的所有 hex 值
    Grounding: source span · direct facts · allowed inference · visual metaphor · must not invent
    文字策略: on-image text policy · exact wording（如有）
...
- Prompt 中的视觉元素必须能回溯到 Source Grounding Ledger：属于直接事实、允许推断或视觉隐喻三者之一
- `Must not invent` 中列出的元素禁止出现在 Prompt 中
- 当目标模型已指定时，优先使用该模型更稳定理解的表述方式，但不得牺牲语义准确性
- 当图中文字策略不是 `必须精确文字` 时，避免在 Prompt 中要求大段可读文本
...
### Source Grounding
- [ ] Prompt 中的关键视觉元素可回溯到 Source Grounding Ledger
- [ ] Direct facts、Allowed inference、Visual metaphor 三者未混淆
- [ ] `Must not invent` 中的元素未出现在 Prompt 中

### 模型与图中文字策略
- [ ] Prompt 写法与目标模型相匹配，未依赖未声明的模型特性
- [ ] 图中文字策略已落实：无字、少量标签、仅数字或精确文字要求一致
...
- ❌ 没有 source span 和 grounding 约束，直接凭印象脑补视觉元素
- ❌ 未声明图中文字策略，却在 Prompt 中要求标题、长段文字或大量标签
