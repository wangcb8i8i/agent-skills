...
共 6 个阶段，按顺序执行。Step 2（理解与产出形态）和 Step 4（风格与配图规划）产出需经用户确认后方可进入下一步。Step 3（风格推断）由 skill 自动完成，用户可在 Step 4 微调。

| # | 步骤 | 需用户确认 | 概要 |
|---|------|-----------|------|
| 1 | INGEST 获取输入 | 否 | 接收文档/URL/粘贴文本 |
| 2 | COMPREHEND + OUTPUT FORM 理解与产出形态 | 是 | 提取领域/受众/关键节点 + 产出形态/模型/文字策略 |
| 3 | STYLE ANCHOR 推断风格 | 否 | skill 基于理解自动推断风格锚点 |
| 4 | PLANNING + STYLE 配图规划与风格确认 | 是 | 确认风格锚点 + 配图规划 + 元素词典 |
...
**将理解的摘要与产出形态规划，一并呈现给用户确认**    示例

```
领域：分布式系统      受众：有后端基础的开发者
类型：教程            语气：技术性 · 严谨 · 逐步引导

候选配图节点（按综合优先级排序）：

  #1  第 2 节 · Leader Election 概念首次出现
      概念密度：高 | 理解增益：高 | 图内文字：无文字

  #2  第 4 节 · Log Replication 架构说明
      概念密度：中 | 理解增益：高 | 图内文字：仅标签

  #3  第 7 节 · 选主 vs 预投票 对比
      概念密度：中 | 理解增益：中 | 图内文字：仅标签

产出形态：[x] 文章内联插图   目标模型：Generic   配图数量：建议 3 张
图内文字策略：尽量无字（标签例外）
```

每个候选节点需包含：概念密度、理解增益、图内文字策略。用户可修正领域/受众判断、增删候选节点、调整优先级和文字策略、修改产出形态/模型/数量。确认后方可进入 Step 3（风格推断）。
...
**使用规则**：上述为建议方向。如果用户在 Step 4（配图规划与风格确认）覆写了风格，以用户覆写为准。

### 3. OUTPUT FORM + MODEL ANCHORING — 产出形态与模型锚定

产出形态、目标模型和全局图内文字策略已在 Step 2 随理解摘要一并提交用户确认。本节内容为 skill 内部使用的参考映射表和约束规则，不单独要求用户确认。
...
从语感/领域/受众推断统一视觉风格，并结合目标模型、优先目标和全局图内文字策略调整风格表达。**不单独要求用户确认**，风格锚点将在 Step 4（配图规划与风格确认）与配图规划一并提交确认。
...
风格锚点必须与 Target Model Anchor 和全局图内文字策略一致。配色方案默认可参考 `references/tag-quick-reference.md`；按配图类型的写法细节见 `references/type-strategies.md`。

### 5. PLANNING + STYLE — 配图规划与风格确认

基于综合优先级驱动，输出配图规划（位置、数量、每张的配图类型），同时呈现风格锚点。每张图自动生成 diagram-slug，并为每张图建立精简的 视觉目标（原文依据与视觉转化方案的融合摘要）。**提交用户确认**，用户可调整风格锚点、增删配图、修改视觉目标描述、调整元素词典。

```
风格锚点
配色方案: ■ 暖琥珀色（强调） · ■ 半透明青色（辅色） · ■ 深海军蓝（背景）
渲染风格: 抽象拓扑 · 矢量扁平 + 微渐变      氛围: 简洁 · 技术感 · 深色
细节密度: 中等

配图规划（共 3 张）：
  #1 [leader-election]     概念引入    视觉目标：中心暖琥珀色六边形（Candidate）
                                        向四周 4 个半透明青色圆形（Follower）
                                        发射虚线箭头，其中 3 个呈绿色确认态，1 个暗态

  #2 [log-replication]     架构说明    视觉目标：水平时间线从左到右展示日志条目追加，
                                        Leader 暖琥珀色接收写入 → 复制到 Follower
                                        半透明青色 → 多数确认后 commit

  #3 [pre-vote-compare]    对比说明    视觉目标：左右分屏，左为原生选主流程，
                                        右为预投票改进，差异区域金色高亮
