...
共 6 个阶段，按顺序执行。Step 2（全文理解）、Step 3（风格锚点）和 Step 4（配图规划）产出需经用户确认后方可进入下一步。

### 1. INGEST — 获取输入

接收用户的文档/上下文。支持：文件路径、URL、粘贴文本、对话上下文。可附带 `--count`、`--type`、`--style-ref` 等可选参数。
...
每张图输出到 `docs/<diagram-slug>.prompt`。可选：输出 `docs/illustration-plan.summary` 汇总文件。
...
将领域、受众、文章类型、语气及候选节点列表输出摘要提交用户确认。用户可修正领域判断、增删候选节点、调整概念密度。确认后方可进入 Step 3。
...
- 配色方案: ■ [English name]([中文名]) [角色] · ■ [English name]([中文名]) [角色] · ■ [English name]([中文名]) [角色]
- 渲染风格:   [当前风格]，可选：写实 · 矢量扁平 · 抽象拓扑 · 等轴测 · 水彩 · 线描 · ……(详见 references/type-strategies.md)
- 氛围基调: [关键词 2-4 个，如 clean · technical · dark · minimal]
- 细节密度: [精致丰富 / 中等细节 / 极简留白]
- 风格参考: [类比或参考方向，如 dark-mode dashboard aesthetic / vintage science textbook]
```
...
- [Candidate node]:  warm amber #FF6B35, hexagon shape, self-illuminated edge glow
- [Follower node]:   translucent cyan #00D4FF at 30% opacity, circle shape
- [RPC message]:     dashed directional arrow, warm amber to cyan
- [consensus]:       overlapping checkmark icons in green #00FF88
```
...
必须按上表顺序排列：Required 8 个 → Conditional（VIEWPOINT / SCALE & DEPTH / TEXTURE / MOTION / FOCUS / ANNOTATIONS）→ EXCLUSIONS → FORMAT。Conditional tag 若不适用则整行删除，不保留空标记。
...
|------|---------------|---------|---------|
| 概念示意图 | 8 | VIEWPOINT, FOCUS | SCALE & DEPTH, TEXTURE |
| 架构图 | 8 | VIEWPOINT, ANNOTATIONS, MOTION | SCALE & DEPTH, TEXTURE |
| 流程图 | 8 | VIEWPOINT, MOTION, SCALE & DEPTH | FOCUS, ANNOTATIONS |
| 信息图 | 8 | VIEWPOINT, ANNOTATIONS, FOCUS | MOTION, SCALE & DEPTH |
...
- 配色方案:     ■ warm amber(暖琥珀色) 主色 · ■ cyan(青色) 辅色 · ■ navy(深蓝黑) 背景色
- 渲染风格:   abstract topology, vector flat with subtle gradients，可选：写实 · 矢量扁平 · 抽象拓扑 · 等轴测 · 水彩 · 线描 · ……
- 氛围基调:        clean · minimal · technical
- 细节密度:     moderate detail, no visual noise
- 风格参考:   dark-mode dashboard aesthetic, translucent self-illuminated nodes
...
- 如有多张图，可以额外输出 `docs/illustration-plan.summary` 汇总文件，列出所有 diagram-slug 和对应位置
...
- [ ] Prompt 块包含全部 8 个 required tag（SUBJECT / COMPOSITION / COLOR PALETTE / LIGHTING / ATMOSPHERE / PROPORTIONS / EXCLUSIONS / FORMAT）
- [ ] Conditional tags 的启用与 Type-Dimension 映射矩阵一致，无遗漏无多余
- [ ] Tag 顺序符合规范：Required 8 → Conditional → EXCLUSIONS → FORMAT
