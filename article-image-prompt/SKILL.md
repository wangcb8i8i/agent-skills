...
| 1 | **INGEST** | 读取原文 | 无 |
| 2 | **AUDIT** | 扫描配图候选 | 无 |
| 3 | **ALIGN** | 确认目录：TRIAGE 选候选 → 方向确认 → REFINE 定风格 | sign-off 确认 |
...
ALIGN 确认"翻译目录"：先选候选、再确认方向、最后定风格。拆为三步 — **TRIAGE**（选哪些章节进译本）、**方向确认门**（确认叙事方向）、**REFINE**（确认每章的设计细节）。

---

#### TRIAGE — 挑选候选

一次展示 AUDIT 输出的全部候选，按置信等级分组。无默认值——每个候选默认丢弃，用户必须明确保留。

```
AUDIT 扫描到以下配图候选：

  🖼  封面: "两阶段提交共识"       [A·清晰度:5 权重:5]
  📄  1. "投票阶段"               [A·清晰度:4 权重:5]
  📄  2. "崩溃恢复时间线"         [B·清晰度:3 权重:4]
  📄  3. "协调器故障转移状态"     [C·清晰度:2 权重:3]

输入候选编号逐个开关（e.g. "+3" 保留 3 / "-2" 去掉 2），完成后键入 sign：
```

封面候选默认保留（它代表文章论点），但用户可用 `-封面` 显式丢弃。

用户响应示例：
- `sign` → 当前保留列表确认，进入方向确认门
- `+2` → 保留候选 2
- `-1` → 去掉候选 1
- `-封面, +2, +3` → 去掉封面，保留 2 和 3

**完成标准：** 用户已键入 `sign`，保留列表不为空。

---

#### 方向确认门

展示保留候选总览，让用户在进入具体风格设计前确认方向。

```text
方向确认 — 以下配图将进入 REFINE 阶段：
...
- **sign** → 进入 REFINE 定风格
- **retry** → 回 AUDIT 重新扫描全文。当前保留候选作为初始约束带入

**完成标准：** 方向已 sign 或已回 AUDIT。
...
- **adjust** → 用户修改风格、替换色板、调整构图，或设置每张图的文字策略。每轮调整后展示完整方案，用户键入 `sign` 确认退出 adjust。
  - ⚠ 如果用户要求增删配图 → 回 TRIAGE 重新确认候选集
