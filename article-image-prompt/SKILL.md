...
展示 AUDIT 输出的全部候选项。根据评分和文章结构推断 2–3 个**组合包**作为选项，用户一次选择即确定配图集：
...
封面始终包含在推荐和精简方案中（它代表文章论点），用户可在自由描述中排除。

组合选项不需附加叙事说明——候选名对文章作者自明。提供 3 个预设选项 + 自由输入让用户选择，**等待用户响应**后再进入下一阶段。按预设选项的内容组织方案描述，不额外输出候选列表（AUDIT 已展示）。
...
根据文章类型推荐**排名靠前的 4 种风格**供用户选择：

| 风格 | 色板标识 | 氛围 |
|------|---------|------|
| tech-clean · 简洁技术风 | `#00D4FF` 主色 | 冷静·精确·技术感 |
| tutorial-warm · 温暖教程风 | `#4FC3F7` 主色 | 明亮·亲切·教学感 |
| analysis-contrast · 高对比分析风 | `#FF3D00` 主色 | 戏剧化·高张力·分析感 |
| product-doc · 产品文档风 | `#1565C0` 主色 | 专业·干净·文档感 |
