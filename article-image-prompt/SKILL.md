...
组合选项不需附加叙事说明——候选名对文章作者自明。**向用户提问选择哪个配图方案**，提供 3 个预设选项 + 自由输入。候选名放在方案描述中，展示本身就是输出，无需先输出再提问。
...
向用户提问选择哪种视觉风格，提供与文章类型最相关的 4 种作为选项（默认选中推荐方案对应的风格），用户也可自由描述：
...
○ 高对比分析风（推荐）
  深底色 + 鲜红高亮 — 像科技媒体封面、大会主视觉
○ 简洁技术风
  深底色 + 青蓝冷调 — 像代码编辑器深色模式
○ 灰度结构风
  浅底色 + 灰阶层次 — 像学术论文图表、工程设计图
○ 温暖教程风
  浅底色 + 蓝橙暖调 — 像在线课程、学习平台插画
○ 其他：_________
```
