...
- **边界性信息**：定义"能做什么、不能做什么"——范围、非目标、约束、必须遵守的约定。**漏读则决策前提崩塌。**
- **决策性信息**：直接影响规划选择——关键发现、已知盲区、未解决问题、重大风险。**漏读则规划方向可能错误。**
- **支撑性信息**：提供证据、背景、细节——代码库具体行为、数据流实际路径、现有模式的表现。**漏读则规划可能不准确，但方向不会反。**

### 排序规则

按优先级自然地组织产物，判断顺序是"先类型，再同一类型内按重要程度"：

1. **边界性信息必须出现在所有决策性信息之前。** 先框定能做什么、不能做什么，再进入具体内容。
2. **决策性信息必须出现在所有支撑性信息之前。** 先暴露影响规划选择的信号，再展开代码库细节。
...
- 任务跨多个子系统且约定繁多 → 产物自然按"约定 → 决策性发现 → 逐子系统细节"展开。

没有"写着写着发现少了哪个节就补上"的操作。如果有多个子系统逐个展开是合理的，那是因为几个子系统各自产生了决策性信息——不是因为"模板中有详细发现这一节"。

**重要——信息分类不是章节标题：** 边界性 / 决策性 / 支撑性 是编写时判断信息类型和排序的标签，不是产物的章节名称。产物标题应使用领域实际名称（模块名、数据流名、问题域术语）——读者读到的是调研结论本身，不是"我在按方法论写作"。
...
- **约束优先。** 所有边界性信息（范围、约束、约定）出现在决策性和支撑性信息之前。不会出现"读者读到一半才发现有个重要的前提被埋在后面"的情况。
