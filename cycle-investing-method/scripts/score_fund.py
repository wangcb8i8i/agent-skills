# -*- coding: utf-8 -*-
"""一键：给任意基金准备好"行业周期投资框架"评分所需的全部材料。
自动完成：解析代码（名称→代码）→ 备齐数据（缓存/实时抓取）→ 计算机械指标
...
设计目的：把"查找→抓取→读多个文件"合并成一条命令，输出即可直接打分。
研究与学习辅助，非投资建议。
"""
...
def resolve(arg):
    ...
    if arg.isdigit():
        ...
        return arg, arg, ""
    cands = [f for f in funds if arg in f["name"] or arg.upper() in f.get("abbr","") or arg.upper() in f.get("pinyin","")]
    if not cands:
        return None, arg, ""
...
def find_data_dir(code):
    """在缓存目录中找基金数据。返回目录或 None。"""
    hit = glob.glob(os.path.join(CACHE, f"{code}_*"))
...
def main():
    ...
    print("=" * 70)
    print(f"行业周期投资框架 · 证据档案")
    print(f"基金：{name}（{code}）  类型：{ftype}")
    print(f"数据：{os.path.relpath(d, ROOT)}")
    print("=" * 70)
    ...
    print("提醒：本评分衡量"与行业周期投资框架的契合度"，非基金优劣，亦非投资建议；")
