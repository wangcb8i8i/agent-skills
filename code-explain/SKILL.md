...
地图不是单张图，是回答"全局之下用户需要知道什么才能做导航决策"的视角组合。
...
选择规则：
- 默认输出**结构+职责**两个视角——绝大多数场景的基线
- 额外视角**按需添加**：仅当基线视角不足以让用户做出"深入哪条分支"的决策时，才追加
- 不自洽才加倍，不自证才加视角——每个额外视角必须有明确的读者需求依据
- 多个视角的输出顺序：**最有助于导航决策的视角在前**
...
- 在分支目录中添加不影响实质理解的分支
- 在输出中使用"轮 X""地图""导航""合拢"等内部流程标签——用户看到的是代码解读，不是解释过程的阶段名
- 在一轮中合并多个地图+导航阶段
